...
forecast = pygame.Surface((screen_width, 9000))
...
def get_icon(description: str, is_day: bool = True, get_background: bool = False, get_color_theme: bool = False) -> Union[tuple[int, int, int], pygame.Surface, bool]:
    ...
    try:
        if 'Snow' in description or 'Blizzard' in description or 'Flurries' in description:
...
def refresh(loc: list, image: pygame.Surface = forecast):
    global bg, lightmode, locationPickerRectCol, textcol, pickingLocation, text, needToUpdate
    ...
    if fontresized.size(periods[0]['detailedForecast'])[0] < screen_width:
        draw_text(periods[0]['detailedForecast'], fontresized, textcol,
                  screen_width / 2, 180)
    else:
        while fontresized.size(periods[0]['detailedForecast'])[0] > screen_width:
    ...
    if str(periods[0]['probabilityOfPrecipitation']['value']) == 'None':
        draw_text('0% rain', fontsmall, textcol, screen_width / 2 + 20, 100)
    else:
    ...
    image.blit(get_icon(periods[0]['shortForecast'],
               periods[0]['isDaytime']), (screen_width / 2 - 250, 50))  # type: ignore

    # Daily forecast
    for i in range(13):
        z = i + 1
        draw_text(''.join([periods[z]['name'], '  ', str(int(''.join([periods[z]['startTime'][5], periods[z]['startTime'][6]]))), '-', str(
            int(''.join([periods[z]['startTime'][8], periods[z]['startTime'][9]])))]), fontsmall, textcol, screen_width / 4, 230 + z * 50)
        draw_text(''.join([str(periods[z]['temperature']), '°']), fontsmall, textcol, screen_width / 4 + 150,
                  230 + z * 50)

        if str(periods[z]['probabilityOfPrecipitation']['value']) == 'None':
            draw_text('0%', fontsmall, textcol,
                      screen_width / 4 + 220, 230 + z * 50)
        else:
            draw_text(''.join([str(periods[z]['probabilityOfPrecipitation']['value']),
                               '%']), fontsmall, textcol, screen_width / 4 + 220, 230 + z * 50)
    ...
    for h in range(156):
        ...
        draw_text(''.join([str(periods_h[h]['temperature']), '°']), fontsmall, textcol, screen_width / 4 + 150,
                  950 + h * 50)

        if str(periods_h[h]['probabilityOfPrecipitation']['value']) == 'None':
            draw_text('0%', fontsmall, textcol,
                      screen_width / 4 + 220, 950 + h * 50)
        else:
            draw_text(''.join([str(periods_h[h]['probabilityOfPrecipitation']['value']),
                               '%']), fontsmall, textcol, screen_width / 4 + 220, 950 + h * 50)
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_RETURN:
                ...
                if pickingLocation == 2:
                    location[0] = abs(int(text))
                    needToUpdate = True
                elif pickingLocation == 3:
                    location[1] = abs(int(text))
                    needToUpdate = True
                    pickingLocation = 0
                    refresh(location, forecast)
